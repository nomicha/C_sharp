...
namespace CS20221223B
{
    internal class Program
    {
                ...
                try
                {
                     ...
                     checked
                    {
                        switch (which)
                        {
                            ...
                            // 12/16 EvenOdd
                            case "E":
                                Console.WriteLine( EvenOdd(RN.Next(0,int.MaxValue)) );
                                //範圍從十位數最小到int的最大值

                                // 12/23 EvenOdd的另一種作法
                                Console.WriteLine(Digists(RN.Next(0, 1000)));
                                break;
        ...
        // 12/23 EvenOdd的另一種作法
        static string Digists(int X)
        {
            string Number = X.ToString();
            int even = 0;
            int odd = 0;
            for (int i = 0; i <= Number.Length - 1; i++)  // Number.Length - 1 是為了讓程式不是只能跑十位數的
            {
                if (int.Parse(Number.Substring(i, 1)) % 2 == 0) even++;   //int.Parse讓本來選出來的字串，轉成數字。
                else odd++;
            }
            return Number + "：偶(" + even + ")奇(" + odd + ")";
        }
    }
}
